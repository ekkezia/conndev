...
void loop() {
  ...
  if (!drawState) return; // do not read and send mqtt sensor msg if drawState is LOW / 'stop'
  ...
  if(now-lastMqttSend>=sendInterval){
    ...
    if(mqttClient.connected()){ // todo: stop sending when drawState 'stop'
      publishMqtt();
    }
...
boolean connectToBroker(){
  ...
  mqttClient.beginWill("kezia/imu/control", true, 0);
  mqttClient.print("{\"power\": false, \"click\":false,\"draw\":\"stop\"}");
  mqttClient.endWill();
...
void publishControl() {
  ...
  // Serial.println(mqttClick);
  mqttClient.beginMessage("kezia/imu/control");
  mqttClient.print("{");
  mqttClient.print("\"power\":");
  mqttClient.print("true"); // false will be handled by last will
