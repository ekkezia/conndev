...
export default function BeatGame({ className }) {
	...
	useEffect(() => {
		...
		function gameLoop(now) {
			...
			// ── Spawn beat ──
			if (
				nextOnsetRef.current !== null &&
				now >= nextOnsetRef.current - SHOW_BEFORE
			) {
			...
			beatsRef.current = beatsRef.current.filter((beat) => {
				...
				if (!beat.hit && !beat.missed) {
					...
					if (inArea && inWindow) {
						...
						const hitColor = new paper.Color({
							hue,
							saturation: 1,
							brightness: 1,
							alpha: 1,
				...
				if (beat.hit || beat.missed) {
					...
					beat.beatCircle.opacity = Math.max(
						0,
						beat.beatCircle.opacity - speed,
					);
					beat.approachCircle.opacity = Math.max(
						0,
						beat.approachCircle.opacity - speed,
					);
				...
				const progress = Math.max(
					0,
					Math.min(1, 1 - timeToOnset / SHOW_BEFORE),
				);
				const r =
					APPROACH_START_RADIUS -
					(APPROACH_START_RADIUS - BEAT_RADIUS) * progress;
				beat.approachCircle.bounds = new paper.Rectangle(
					beat.x - r,
					beat.y - r,
					r * 2,
					r * 2,
				...
				// Onset pulse
				const pulseR =
					Math.abs(timeToOnset) < 80
						? BEAT_RADIUS * (1 + 0.15 * (1 - Math.abs(timeToOnset) / 80))
						: BEAT_RADIUS;
				beat.beatCircle.bounds = new paper.Rectangle(
					beat.x - pulseR,
					beat.y - pulseR,
					pulseR * 2,
					pulseR * 2,
		...
		<div className={`absolute top-0 left-0 w-full h-full ${className ?? ''}`}>
			...
			{activeSong && (
				<div className="absolute top-2 left-1/2 -translate-x-1/2 bg-black/60 text-white font-mono text-xs px-4 py-1.5 rounded-full flex items-center gap-4">
					...
					<span className="text-fuchsia-400 font-bold tabular-nums pointer-events-none">
						{score} pts
					</span>
					<button
						type="button"
