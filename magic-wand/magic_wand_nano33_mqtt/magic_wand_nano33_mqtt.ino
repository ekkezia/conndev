...
void handleFeedbackMessage(const String& msgRaw) {
  ...
  if (normalized == "hover") {
    ...
    playHaptic(HAPTIC_TICK);
  } else if (normalized == "beat_hit missed") {
    playHaptic(HAPTIC_SOFT_BUMP);
  } else if (normalized.indexOf("\"type\":\"hover\"") >= 0) {
    playHaptic(HAPTIC_TICK);
  } else if (normalized.indexOf("\"type\":\"click\"") >= 0) {
    playHaptic(HAPTIC_CLICK);
    ...
    if (normalized.indexOf("\"state\":\"perfect\"") >= 0) {
      ...
      playHaptic(HAPTIC_TICK);
    } else if (normalized.indexOf("\"state\":\"missed\"") >= 0) {
      playHaptic(HAPTIC_SOFT_BUMP);
    }
  }
}
...
void handleInboundMqtt() {
  ...
  if (topicName == topicFeedback) {
    Serial.print("MQTT feedback: ");
